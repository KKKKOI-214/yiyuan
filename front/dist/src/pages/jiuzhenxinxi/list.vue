	...
	<div class="list" :style='{"minHeight":"900px","padding":"20px 0","margin":"0px 0 0","borderRadius":"0px","background":"none","width":"calc(100% - 400px)","float":"left","order":"6"}'>
		...
		<div :style='{"width":"100%","padding":"0 0 0 2%","flexWrap":"wrap","background":"none","display":"flex","height":"auto"}' class="list list4 index-pv1">
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" :style='{"width":"31.3%","margin":"0 2% 30px 0","overflow":"hidden","position":"relative","background":"#fff","height":"480px"}' class="item">
			...
			<div class="content">
		      <div :style='{"padding":"30% 10px 20px 0","borderColor":"#ddd","color":"inherit","alignItems":"center","display":"block","textIndent":"2em","overflow":"hidden","borderWidth":"0 1px 0 0","width":"48%","lineHeight":"30px","fontSize":"inherit","borderStyle":"dashed","height":"100%"}'>{{item.yishengjianjie}}</div>
		      <div class="info" :style='{"padding":"30% 0 0 0px","overflow":"hidden","alignItems":"flex-start","flexWrap":"wrap","display":"block","width":"48%","height":"100%"}'>
				...
				<div :style='{"padding":"0 0 0 20px","borderColor":"#ddd","whiteSpace":"nowrap","color":"inherit","textAlign":"left","overflow":"hidden","borderWidth":"0 0 0px","width":"100%","lineHeight":"40px","fontSize":"inherit","textOverflow":"ellipsis","borderStyle":"dashed","fontWeight":"600","height":"auto"}' class="name ">{{item.zhiwei}}</div>
				<div :style='{"padding":"0 0 0 20px","borderColor":"#ddd","whiteSpace":"nowrap","color":"inherit","textAlign":"left","overflow":"hidden","borderWidth":"0 0 0px","width":"100%","lineHeight":"40px","fontSize":"inherit","textOverflow":"ellipsis","borderStyle":"dashed","fontWeight":"600","height":"auto"}' class="name ">余号:{{item.yuhao}}</div>
...
<script>
  export default {
    ...
    created() {
		...
		this.$http.get('option/keshixinxi/keshimingcheng').then(res => {
			if (res.data.code == 0) {
				this.keshimingchengOptions = res.data.data;
				this.selectOptionsList.push({name:'科室名称',list:this.keshimingchengOptions,tableName: 'keshimingcheng',check: -1})
			}
		});
      this.getFenlei();
      this.getList(1, '全部');
    ...
    methods: {
      ...
      getFenlei() {
		this.$http.get('keshixinxi/list',{}).then(res => {
		  if (res.data.code == 0) {
		    this.fenlei = res.data.data.list
		  }
		});
		this.feileiColumn = 'keshimingcheng'
      },
      getList(page, fenlei, ref = '') {
        ...
        if (this.curFenlei != '全部') searchWhere.keshimingcheng = this.curFenlei;
