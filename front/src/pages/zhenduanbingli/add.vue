...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"none"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="诊断时间" prop="zhenduanshijian">
              <el-date-picker
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="收缩压mmHg" prop="shousuoya">
			<el-input-number v-model="ruleForm.shousuoya" placeholder="收缩压mmHg" :disabled="ro.shousuoya"></el-input-number>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="舒张压mmHg" prop="shuzhangya">
			<el-input-number v-model="ruleForm.shuzhangya" placeholder="舒张压mmHg" :disabled="ro.shuzhangya"></el-input-number>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="体温/°" prop="tiwen">
			<el-input-number v-model="ruleForm.tiwen" placeholder="体温/°" :disabled="ro.tiwen"></el-input-number>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="病历图片" v-if="type!='cross' || (type=='cross' && !ro.binglitupian)" prop="binglitupian">
            <file-upload
            tip="点击上传病历图片"
            action="file/upload"
            :limit="3"
            :multiple="true"
            :fileUrls="ruleForm.binglitupian?ruleForm.binglitupian:''"
            @change="binglitupianUploadChange"
            ></file-upload>
          </el-form-item>
            <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' class="upload" v-else label="病历图片" prop="binglitupian">
                <img v-if="ruleForm.binglitupian.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.binglitupian.split(',')[0]" width="100" height="100">
                <img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.binglitupian.split(',')" :src="baseUrl+item" width="100" height="100">
            </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="诊断报告" prop="zhenduanbaogao">
            <file-upload
            tip="点击上传诊断报告"
            action="file/upload"
            :limit="1"
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="病情登记" prop="bingqingdengji">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="病情登记"
              v-model="ruleForm.bingqingdengji">
            </el-input>
          </el-form-item>
...
<script>
  export default {
    data() {
      ...
      return {
        ...
        rules: {
          ...
          shousuoya: [
            { required: true, message: '收缩压mmHg不能为空', trigger: 'blur' },
            { validator: this.$validate.isNumber, trigger: 'blur' },
          ],
          shuzhangya: [
            { required: true, message: '舒张压mmHg不能为空', trigger: 'blur' },
            { validator: this.$validate.isNumber, trigger: 'blur' },
          ],
          tiwen: [
            { required: true, message: '体温/°不能为空', trigger: 'blur' },
            { validator: this.$validate.isNumber, trigger: 'blur' },
          ],
          binglitupian: [
          ],
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='binglitupian'){
              this.ruleForm.binglitupian = obj[o].split(",")[0];
              this.ro.binglitupian = true;
              continue;
            }
            if(o=='bingqingdengji'){
              this.ruleForm.bingqingdengji = obj[o];
              this.ro.bingqingdengji = true;
              continue;
            }
            if(o=='zhenduanbaogao'){
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`zhenduanbingli/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('zhenduanbingli/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`zhenduanbingli/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
      ...
      binglitupianUploadChange(fileUrls) {
          this.ruleForm.binglitupian = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
      },
      zhenduanbaogaoUploadChange(fileUrls) {
          this.ruleForm.zhenduanbaogao = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
      },
    }
  };
</script>
