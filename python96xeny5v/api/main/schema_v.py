...
@main_bp.route("/python96xeny5v/cal/<tableName>/<columnName>", methods=['GET'])
def python96xeny5v_cal(tableName, columnName):
    '''
    计算规则接口
    '''
...
@main_bp.route("/python96xeny5v/file/download", methods=['GET'])
def python96xeny5v_file_download():
    '''
    '''
    if request.method == 'GET':
...
@main_bp.route("/python96xeny5v/file/upload", methods=['POST'])
def python96xeny5v_file_upload():
    '''
    '''
...
@main_bp.route("/python96xeny5v/follow/<tableName>/<columnName>/<level>/<parent>", methods=['GET'])
def python96xeny5v_follow_level(tableName, columnName, level,parent):
    '''
    '''
    if request.method == 'GET':
        msg = {"code": normal_code, 'data': []}
...
@main_bp.route("/python96xeny5v/follow/<tableName>/<columnName>", methods=['GET'])
def python96xeny5v_follow(tableName, columnName):
    '''
...
@main_bp.route("/python96xeny5v/location", methods=['GET'])
def python96xeny5v_location():
    '''
...
@main_bp.route("/python96xeny5v/matchFace", methods=['GET'])
def python96xeny5v_matchface():
    '''
    baidubce百度人脸识别
    '''
    if request.method == 'GET':
...
@main_bp.route("/python96xeny5v/option/<tableName>/<columnName>", methods=['GET'])
def python96xeny5v_option(tableName, columnName):
    '''
    '''
    if request.method == 'GET':
        msg = {"code": normal_code, 'data': []}
...
@main_bp.route("/python96xeny5v/remind/<tableName>/<columnName>/<type>", methods=['GET'])  #
def python96xeny5v_remind(tableName, columnName,type):
    '''
    '''
    if request.method == 'GET':
        msg = {"code": normal_code, 'data': []}
...
@main_bp.route("/python96xeny5v/sh/<tableName>", methods=['POST'])
def python96xeny5v_sh(tableName):
    '''
    '''
    if request.method == 'POST':
        ...
        logging.warning("python96xeny5v_sh.res=====>{}".format(res))
        return jsonify(msg)

@main_bp.route("/python96xeny5v/upload/<fileName>", methods=['GET'])
def python96xeny5v_upload(fileName):
    '''
    上传接口
    '''
    if request.method == 'GET':
...
@main_bp.route("/python96xeny5v/group/<tableName>/<columnName>", methods=['GET'])
...
@main_bp.route("/python96xeny5v/value/<tableName>/<xColumnName>/<yColumnName>", methods=['GET'])
...
@main_bp.route("/python96xeny5v/info/ai", methods=['GET'])
def schema_value_info_ai():
    if request.method in ['GET', 'POST']:
        try:
            ...
            headers = {
                "Authorization": "",  # Add your authorization if needed
            ...
            regex = r"(?is)<think[^>]*>.*?</think>"
            ai_response = re.sub(regex, "", ai_response, count=1).strip()
            print(ai_response)
