...
<div class="content" :style='{"minHeight":"100vh","padding":"40px 30px 20px","alignItems":"flex-start","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20231104/93c9cca5595b4e21bb2a11e440cdca8d.png) no-repeat center top / cover,#f6f6f6","display":"flex","fontSize":"14px","height":"100%"}'>
	...
	<!-- statis -->
	<div :style='{"border":"0px solid rgba(126, 96, 16, .1)","padding":"0px 0","margin":"0 0px 20px 30px","alignItems":"center","color":"#7331f2","flexWrap":"wrap","background":"none","display":"flex","width":"200px","fontSize":"14px","justifyContent":"center","order":"2"}'>
		<div :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","padding":"8px 0","margin":"0 0 10px 0","borderColor":"#eee","alignItems":"center","textAlign":"left","display":"flex","justifyContent":"space-around","borderRadius":"0px","background":"#fff","borderWidth":"1px","width":"100%","borderStyle":"solid"}' v-if="isAuth('yuyueguahao','首页总数')">
			...
			<div :style='{"width":"auto","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"24px","color":"inherit","fontWeight":"bold","height":"24px"}'>{{yuyueguahaoCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"inherit","color":"inherit","height":"24px"}'>预约挂号总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<div class="type3" :style='{"alignContent":"flex-start","padding":"0","borderRadius":"8px","flexWrap":"wrap","background":"none","flex":"1","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto","order":"10"}'>
		<div id="yuyueguahaoChart1" class="echarts1" v-if="isAuth('yuyueguahao','首页统计')"></div>
		<div id="yuyueguahaoChart2" class="echarts2" v-if="isAuth('yuyueguahao','首页统计')"></div>
		<div id="yuyueguahaoChart3" class="echarts3" v-if="isAuth('yuyueguahao','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	methods:{
		...
		getyuyueguahaoCount() {
			// 生成随机预约总数
			this.yuyueguahaoCount = Math.floor(Math.random() * 1000) + 500; // 500-1500之间的随机数
		},
		yuyueguahaoChat1() {
			this.$nextTick(()=>{

        var yuyueguahaoChart1 = echarts.init(document.getElementById("yuyueguahaoChart1"),'blue');
        ...
        const generateDailyData = () => {
            ...
            for(let i = 0; i < days.length; i++){
                ...
                const randomValue = Math.floor(Math.random() * 100) + 20; // 20-120之间的随机数
                xAxis.push(days[i]);
                yAxis.push(randomValue);
                pArray.push({
                    value: randomValue,
                    name: days[i]
                });
            ...
            return { xAxis, yAxis, pArray };
        };
        
        const { xAxis, yAxis, pArray } = generateDailyData();
        
        var option = {};
		let titleObj = this.bar.title
		titleObj.text = '每日预约人数统计'
    ...
    yuyueguahaoChat2() {
      this.$nextTick(()=>{
        ...
        var yuyueguahaoChart2 = echarts.init(document.getElementById("yuyueguahaoChart2"),'blue');
        ...
        const generateGenderData = () => {
            ...
            for(let i = 0; i < genders.length; i++){
                ...
                const randomValue = Math.floor(Math.random() * 300) + 100; // 100-400之间的随机数
                xAxis.push(genders[i]);
                yAxis.push(randomValue);
                pArray.push({
                    value: randomValue,
                    name: genders[i]
        ...
        const { xAxis, yAxis, pArray } = generateGenderData();
        
        var option = {};
                let titleObj = this.pie.title
				titleObj.text = '用户性别统计'
    ...
    yuyueguahaoChat3() {	
        ...
        var yuyueguahaoChart3 = echarts.init(document.getElementById("yuyueguahaoChart3"),'blue');
        ...
        const generateDepartmentData = () => {
            ...
            for(let i = 0; i < departments.length; i++){
                ...
                const randomValue = Math.floor(Math.random() * 150) + 30; // 30-180之间的随机数
                xAxis.push(departments[i]);
                yAxis.push(randomValue);
                pArray.push({
                    value: randomValue,
                    name: departments[i]
        ...
        const { xAxis, yAxis, pArray } = generateDepartmentData();
        
        var option = {};
                let titleObj = this.line.title
				titleObj.text = '科室类型统计'
                ...
                // 使用刚指定的配置项和数据显示图表。
                yuyueguahaoChart3.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    yuyueguahaoChart3.resize();
                };
            }
      	}
	};
