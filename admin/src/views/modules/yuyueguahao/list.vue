<template>
	<div class="main-content" :style='{"padding":"20px 30px 30px 0","fontSize":"14px","color":"#666","background":"#f6f6f6"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"20px 30px","margin":"0 0 0 30px","overflow":"hidden","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"calc(100% - 30px)","fontSize":"inherit","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"margin":"0 0 10px","textAlign":"left","flexWrap":"wrap","display":"flex","width":"auto","fontSize":"inherit","order":"1"}' >
					<div :style='{"margin":"0 0 0 0","fontSize":"inherit","alignItems":"center","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"none","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">医生姓名</label>
						<el-input v-model="searchForm.yishengxingming" placeholder="医生姓名" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 0 0 0","fontSize":"inherit","alignItems":"center","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"none","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">职位</label>
						<el-input v-model="searchForm.zhiwei" placeholder="职位" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 0 0 0","fontSize":"inherit","alignItems":"center","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"none","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">科室名称</label>
				...
				<el-row class="actions" :style='{"margin":"0 0 10px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"auto","fontSize":"inherit","order":"2"}'>
					...
					<el-button class="btn18" v-if="isAuth('yuyueguahao','每日预约人数统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						每日预约人数统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('yuyueguahao','用户性别统计')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						用户性别统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('yuyueguahao','科室类型统计')" type="success" @click="chartDialog3()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						科室类型统计
					</el-button>
				</el-row>
			...
			<div :style='{"border":"0px solid #eee","width":"calc(100% - 30px)","padding":"0 30px 30px","margin":"0px 0 0 30px","borderRadius":"0px","background":"rgba(255,255,255,1)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="zf" label="支付状态">
						<template slot-scope="scope">
							<el-tag v-if="scope.row.zf=='已支付'" type="danger">已支付</el-tag>
							<el-tag v-if="scope.row.zf==''||scope.row.zf==null" type="warning">未支付</el-tag>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('yuyueguahao','诊断')" @click="zhenduanbingliCrossAddOrUpdateHandler(scope.row,'cross','是','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								诊断
							</el-button>
							<el-button class="btn8" v-if="isAuth('yuyueguahao','取消')" @click="yuyuequxiaoCrossAddOrUpdateHandler(scope.row,'cross','否','','[1]','已申请取消')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								取消
							</el-button>
							<el-button class="btn8" v-if="isAuth('yuyueguahao','评价')" @click="pingjiaxinxiCrossAddOrUpdateHandler(scope.row,'cross','是','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								评价
							</el-button>
		...
		<zhenduanbingli-cross-add-or-update v-if="zhenduanbingliCrossAddOrUpdateFlag" :parent="this" ref="zhenduanbingliCrossaddOrUpdate"></zhenduanbingli-cross-add-or-update>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="keshimingchengChart3" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
import zhenduanbingliCrossAddOrUpdate from "../zhenduanbingli/add-or-update";
	...
	export default {
		data() {
			return {
				...
				zhenduanbingliCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			zhenduanbingliCrossAddOrUpdate,
		...
		methods: {
			...
			zhenduanbingliCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.zhenduanbingliCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.zhenduanbingliCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.zhenduanbingliCrossaddOrUpdate.init(row.id,type);
				});
			},
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var yuyueshijianChart1 = echarts.init(document.getElementById("yuyueshijianChart1"),'blue');
        this.$http({
            url: "yuyueguahao/group/yuyueshijian",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
...
//统计接口
    chartDialog2() {
      ...
      this.$nextTick(()=>{
        ...
        var xingbieChart2 = echarts.init(document.getElementById("xingbieChart2"),'blue');
        this.$http({
            url: "yuyueguahao/group/xingbie",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
...
//统计接口
    chartDialog3() {
      ...
      this.$nextTick(()=>{
        ...
        var keshimingchengChart3 = echarts.init(document.getElementById("keshimingchengChart3"),'blue');
        this.$http({
            url: "yuyueguahao/group/keshimingcheng",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                keshimingchengChart3.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    keshimingchengChart3.resize();
                };
            }
        });
      })
