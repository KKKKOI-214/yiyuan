		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"20px 30px","margin":"0 0 0 30px","overflow":"hidden","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"calc(100% - 30px)","fontSize":"inherit","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"margin":"0 0 10px","textAlign":"left","flexWrap":"wrap","display":"flex","width":"auto","fontSize":"inherit","order":"1"}' >
					<div :style='{"margin":"0 0 0 0","fontSize":"inherit","alignItems":"center","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"none","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">疾病名称</label>
						<el-input v-model="searchForm.jibingmingcheng" placeholder="疾病名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 0 0 0","fontSize":"inherit","alignItems":"center","display":"flex"}' class="select" label="症状类型" prop="zhengzhuangleixing">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"none","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">症状类型</label>
						<el-select clearable v-model="searchForm.zhengzhuangleixing" placeholder="请选择症状类型" >
							<el-option v-for="(item,index) in zhengzhuangleixingOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"margin":"0 0 0 0","fontSize":"inherit","alignItems":"center","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"none","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">目标人群</label>
						<el-input v-model="searchForm.mubiaorenqun" placeholder="目标人群" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 0 0 0","fontSize":"inherit","alignItems":"center","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"none","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">发布日期</label>
				...
				<el-row class="actions" :style='{"margin":"0 0 10px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"auto","fontSize":"inherit","order":"2"}'>
					<el-button class="add" v-if="isAuth('jibingxinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia7" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						增加
					</el-button>
					<el-button class="del" v-if="isAuth('jibingxinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"0px solid #eee","width":"calc(100% - 30px)","padding":"0 30px 30px","margin":"0px 0 0 30px","borderRadius":"0px","background":"rgba(255,255,255,1)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="zhengzhuangleixing"
						label="症状类型">
						<template slot-scope="scope">
							{{scope.row.zhengzhuangleixing}}
						</template>
					</el-table-column>
					<!-- 无 -->
					<el-table-column :resizable='true' :sortable='true' prop="jibingfengmian" width="200" label="疾病封面">
						<template slot-scope="scope">
							<div v-if="scope.row.jibingfengmian">
								<img v-if="scope.row.jibingfengmian.substring(0,4)=='http'" :src="scope.row.jibingfengmian.split(',')[0]" width="100" height="100" style="object-fit: cover">
								<img v-else :src="$base.url+scope.row.jibingfengmian.split(',')[0]" width="100" height="100" style="object-fit: cover">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('jibingxinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="edit" v-if=" isAuth('jibingxinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('jibingxinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
import AddOrUpdate from "./add-or-update";
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.zhengzhuangleixing!='' && this.searchForm.zhengzhuangleixing!=undefined){
            params['zhengzhuangleixing'] = this.searchForm.zhengzhuangleixing
          }
           if(this.searchForm.mubiaorenqun!='' && this.searchForm.mubiaorenqun!=undefined){
            params['mubiaorenqun'] = '%' + this.searchForm.mubiaorenqun + '%'
          }
	...
	jibingxinxistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'jibingxinxi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
